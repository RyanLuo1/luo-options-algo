...
4:3, right column, `.hero-slot`. Replace the `<div>` with `<img class="hero-slot" src alt>`; the card cluster is absolutely positioned over it, so the swap changes no layout. On phones the two back cards hide and the front card stays over the slot.
