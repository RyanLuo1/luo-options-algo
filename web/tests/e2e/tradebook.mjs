...
try {
  ...
  await page.waitForFunction(() => !/Loading chain…/.test(document.body.textContent), null, { timeout: 60000 })
  ...
  log('pending panel note', /Grading pending · grades after the next close/.test(await page.textContent('body')))
  ...
  log('graded panel: settled marker, Realized P&L card, settlement sentence, read-only editor action', /settled at \$1,040\.00/.test(panel) && /Realized P&L/.test(panel) && /Settled at \$1,040\.00 — you captured the full spread \(\$1,585, max profit\)/.test(panel) && /View in editor/.test(panel))
