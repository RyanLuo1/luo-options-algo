...
try {
  ...
  const markLogin = await page.evaluate(() => { const r = document.querySelector('header a[aria-label="Trigram home"]').getBoundingClientRect(); return [Math.round(r.x), Math.round(r.y), Math.round(r.height)] })
  await page.locator('button[type="submit"]').click()
  await page.waitForURL(/\/app$/, { timeout: 30000 }); await page.waitForSelector('header button[aria-label="Trigram, go to the screener"]', { timeout: 15000 })
  const markApp = await page.evaluate(() => { const r = document.querySelector('header button[aria-label="Trigram, go to the screener"]').getBoundingClientRect(); return [Math.round(r.x), Math.round(r.y), Math.round(r.height)] })
