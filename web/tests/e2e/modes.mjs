...
try {
  ...
  log('no probability control in Income either: four fields + Run scan', (await page.locator('#lc-min-p').count()) === 0 && (await page.locator('section[aria-label="Scan controls"] input:not([type="range"])').count()) === 3 && (await page.locator('section[aria-label="Scan controls"] input[type="range"]').count()) === 2)   // tickers, credit, the gate field + the two slider thumbs
  const bodies = []; page.on('request', r => { if (r.url().endsWith('/api/run')) bodies.push(r.postData()) })
  ...
  log('the Income request carries its return-on-collateral floor (1%) so the server logs the shown list', bodies.length === 1 && Math.abs(JSON.parse(bodies[0]).min_roc - 0.01) < 1e-9, bodies[0])
  ...
  log('Upside gate row: minimum upside per $ of collateral at 5%, credit at $0, no probability gate', (await page.locator('#lc-min-upside').inputValue()) === '5' && (await page.locator('#lc-min-credit').inputValue()) === '0' && (await page.locator('#lc-min-roc').count()) === 0 && (await page.locator('#lc-min-p').count()) === 0)
  // ── the credit floor may go negative in Upside only (net-debit structures inspectable)
  await page.fill('#lc-min-credit', '-200'); await page.locator('#lc-min-credit').blur(); await page.waitForTimeout(100)
  log('Upside accepts a negative credit floor', (await page.locator('#lc-min-credit').inputValue()) === '-200' && !/Enter (0 or more|a number)\./.test(await page.textContent('section[aria-label="Scan controls"]')) && !(await page.getByRole('button', { name: /Run scan/ }).isDisabled()))
  await page.fill('#lc-min-credit', '0'); await page.locator('#lc-min-credit').blur()
  await page.getByRole('tab', { name: 'Income' }).click(); await page.waitForTimeout(150)
  await page.fill('#lc-min-credit', '-1'); await page.waitForTimeout(100)
  log('Income rejects a negative credit floor', /Enter 0 or more\./.test(await page.textContent('section[aria-label="Scan controls"]')) && (await page.getByRole('button', { name: /Run scan/ }).isDisabled()))
  await page.fill('#lc-min-credit', '100'); await page.locator('#lc-min-credit').blur()
  await page.getByRole('tab', { name: 'Upside' }).click(); await page.waitForTimeout(150)
  bodies.length = 0
  ...
  log('Upside requests carry no return floor (unchanged)', bodies.every(b => !('min_roc' in JSON.parse(b))))
  ...
  await page.locator('#lc-min-roc-tip').locator('..').locator('button').hover(); await page.waitForTimeout(150)
  const tipBox = await page.locator('#lc-min-roc-tip').boundingBox()
  log('the rightmost ⓘ tooltip stays inside the viewport', !!tipBox && tipBox.x >= 0 && tipBox.x + tipBox.width <= 1440, tipBox ? `right edge ${Math.round(tipBox.x + tipBox.width)}` : 'no tooltip')
