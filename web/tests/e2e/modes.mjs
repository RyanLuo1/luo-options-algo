...
try {
  ...
  log('gauge shows Chance of max profit ≈ δ_B', /Chance of max profit ≈ \d+%/.test(await page.textContent('section[aria-label^="Setup detail"]')))
  ...
  log('selecting the Upside pick drives the panel with its pills', /Upside/.test(panel) && /Not backtested — calculator only/.test(panel))
