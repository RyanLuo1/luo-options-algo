...
// scan set. Tickers with setups sit together on the first line; the ones that
// didn't make the cut sit under them, each with its cause in words, as the
...
export default function ScanChips({ tickers, counts, reasons, details, skipped, activeFilter, onToggle, onRemove, relaxedAvailable, relaxedOpen, onToggleRelaxed }) {
  ...
  // The tickers with setups sit together on the first line; the ones that didn't make the cut
  // sit under them, one per line with their cause, so the scan reads as two answers, not a jumble.
  const hits = (tickers ?? []).filter(t => (counts?.[t] ?? 0) > 0)
  const misses = (tickers ?? []).filter(t => (counts?.[t] ?? 0) === 0)
  const chip = t => {
    ...
    return (
      <span key={t} data-chip={t} className="inline-flex items-center gap-2 flex-wrap">
        <span
  ...
  return (
    <div className="flex flex-col gap-2 px-1" aria-label="Tickers in this scan">
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[0.85rem] font-semibold text-lc-ink-2 mr-1">Scanning</span>
        {hits.map(chip)}
        {hits.length === 0 && misses.length > 0 && <span className="text-[0.85rem] text-lc-ink-2">no ticker produced a setup</span>}
        {activeFilter && (
      ...
      {misses.length > 0 && (
        <div className="flex items-start gap-2 flex-wrap" data-misses>
          <span className="text-[0.85rem] font-semibold text-lc-ink-2 mr-1 h-8 inline-flex items-center">Didn’t make the cut</span>
