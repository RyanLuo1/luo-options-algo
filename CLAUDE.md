...
**Fleet completion runbook** (run in order when the fleet range finishes): (1) confirm `extract_claims` shows every trading day of 2025-08-04 → 2026-07-25 `done`; (2) Mac mirror verification against the manifest — file count AND per-file sizes matching `parquet_bytes` for the FULL range; (2.5) if the t3 catch-up backlog persists at fleet completion, point the idle c7i at the remaining live catch-up dates before termination — an afternoon of fleet-speed catch-up vs days of the t3 treading water at ~1 file/day; (3) only after (2) verifies clean (and 2.5 if applicable), terminate the box.
