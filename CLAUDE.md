...
- **`luo-capital-screener-state`** — App.jsx control state, written via a single `useEffect` whose deps include every persisted field. Persisted fields:
  ...
  - `cardFilter`, `selectedSetup`, `chartTimeframe` (detail-panel filter + row override; both `null` → panel shows the overall rank-1 default)
...
The screener page (`App.jsx` route `/`) is locked to viewport height so the Header, control bar (with the folded-in scanning chips), the per-ticker overview strip, and the table's metadata bar always stay in view. Only the table body (and the overview strip, horizontally) scrolls. The macro-events band and the table legend are **collapsed by default** (toggled from the controls / metadata bar respectively), so they only occupy space when explicitly expanded.

The three zones stack vertically inside the locked page: **controls** (`shrink-0`), the **per-ticker overview strip** (`shrink-0`, fixed height — one horizontal row), and the **detail zone** (`<main>`, `flex-1`), which is itself a horizontal flex row of **two columns**: the **ranked table** (left, `w-[33rem] shrink-0` — it **hugs its columns**, the 33rem column sum, so Max Profit sits at the right edge with no trailing dead space; ~38% on a typical laptop) and the **right detail zone** (`flex-1 min-w-[360px]` — takes all remaining width, so the chart gets the most room) where **`SetupDetail` stacks ABOVE the chart** (SetupDetail is a `shrink-0` band; the chart `flex-1` fills the remaining height, so there's no empty void). When the chart is fullscreen (`isChartFull`), the table column is not rendered and the chart fills the whole zone.
...
- **`App.jsx`** — screener page only (not a router/layout); owns all scan state, chart state, and control logic
  - **Card-as-filter + per-setup detail model.** Two state atoms drive the detail panel: `cardFilter` (the ticker an overview card has focused on — `null` = no filter; also drives which card gets the accent) and `selectedSetup` (an explicit table-row override — `null` = use the context default). Everything else is **derived**:
    - `baseRanked` = `ranked` filtered by the Holdings `activeTickers`; `tableRows` = `baseRanked` further filtered to `cardFilter` (if set), re-ranked `1..N`.
    - `contextDefault` = `cardFilter ? byTicker.find(cardFilter).best : baseRanked[0]` (the overall rank-1).
    - `displayedSetup` = `selectedSetup ?? contextDefault` → feeds `SetupDetail`; `chartTicker = displayedSetup?.ticker` feeds the chart; `selectedKey` (matches `ResultsTable.rowKey`) highlights its row.
  - **Behavior:** on a new scan a `useEffect` (primed `lastRankedRef`) resets `cardFilter` and `selectedSetup` to `null` → the detail panel **defaults to the overall rank-1 setup** while **no card has the accent** and the table shows the full list. `toggleCardFilter(ticker)` (card click) toggles the single-ticker filter on/off/switch and clears the row override → table filters to that ticker, detail shows its best, that card gets the accent; re-clicking the same card clears the filter (back to overall rank-1, no accent). `selectRow(row)` (row click) sets `selectedSetup` → detail shows that specific setup, overriding the default **within** the current filter (the card stays the filter/accent). Removing a Holdings pill that is the `cardFilter` / override clears it.
  - App also owns `chartFull` (default `false`, not persisted) — the chart's `⤢` button expands it to **fill the whole detail zone** (the ranked-table column is hidden via `isChartFull = chartFull && !!chartTicker`), and `⤡` collapses it back to the two-column layout. There is **no close (`×`)** — the detail panel always shows a setup (the rank-1 default or the filtered/selected one). `chartFull` resets to `false` on a new scan and on Clear.
  - Calls `useChartData(chartTicker, chartTimeframe)`; the result feeds the single chart in the detail panel. `cardFilter` and `selectedSetup` are persisted to sessionStorage.
  - **Controls** live in a single **contained surface panel** (`bg-surface`, rounded, `border-subtle`) with a small uppercase `CONTROLS` label and the `macro ⌄ / ⌃` toggle at the panel's right edge. The four fields — Tickers, Weeks range, Min Net Premium, Min P(Profit) — are evenly-spaced **contained controls** (each on `bg-surface-raised` with a `border-subtle`, label above, `focus`/`focus-within:border-accent`). Per-field guidance lives in **placeholders + `title` tooltips** (no helper lines). The folded-in scanning chips render as a sub-row **inside** the panel, divided by a `border-subtle` top rule.
...
- **`Header.jsx`** — route-aware header (uses `useLocation`); rendered independently by each page component:
  - `/` (screener): a **contained surface panel** (`bg-surface`, rounded, `border-subtle`). Branding (`Luo Capital / Options Screener`) sits left; the right side is **one tidy action cluster**, evenly spaced and vertically centered: market badge → Clear → **Run Scan** (the `bg-accent` primary, amber when stale) → Tradebook → a divided, muted `Last run {timestamp}` + `Log out`. Nothing floats in the corner.
...
- **`SetupDetail.jsx`** — the rich detail block for the selected setup, rendered as a compact `shrink-0` **band at the top of the right detail column**, stacked **above the chart** (the chart `flex-1` fills the height below it, so there's no empty void). Shows: setup stats (collected / max profit / score 2 dp / P(profit) 1 dp) in a wrapping row, the **full three-leg breakdown** (`leg` / strike / prem / delta per leg, color-coded per convention — **Leg A long call = sky** (you pay), **Leg B short call & Leg C short put = profit token** (you collect)), spread + fair value, and the **Save** / **Open in editor ›** actions (wired to App's `saveToTradebook(displayedSetup)` / `handleEdit(displayedSetup)`). When nothing is selected the right column shows a subtle "select a setup…" placeholder. Pure presentation; tokens only; per-contract dollars are `× 100`.
...
- **`OverviewCard.jsx`** — one ticker's single best setup, rendered as a rich "top-pick" card for the per-ticker overview section (above the results table). Driven by a `by_ticker[i]` entry (`{ ticker, best, count }`). Pure presentation, **all colors/fonts from design tokens** (`bg-surface`, `border-subtle`/`border-accent`, `text-profit`, `loss-dim`/`profit-dim`, `.num`) — no hardcoded hex.
  - **Header:** ticker (bold) + `expiration · W{week}` (muted) on the left; on the right the **net credit collected per contract** = `net_premium × 100`, shown in the profit token as `+$2,044` (`.num`) with a muted `collected / contract` label.
  - **Payoff-zone bar:** a horizontal bar split into three token-colored zones by strike — loss (`loss-dim`, far left → put strike `leg_c_strike`), credit-only (`surface-raised`, a faint neutral fill distinct from the card's `surface` background so the zone reads as a connected segment rather than an empty gap, put → long call `leg_a_strike`), max (`profit-dim`, long call → right through `leg_b_strike`). Bounds are ~10% below the put strike to ~10% above the short call so all zones are visible. A vertical `accent` **"now" marker** sits at `underlying_price` with a small price label. Under-bar labels `loss <$[put]` / `max zone` (whole-dollar strikes) are **dropped in compact mode** to avoid collision at ~330px.
  - **No stat row.** The card is intentionally light — only the ticker + `expiration · W{week}`, the collected-credit header (`+$2,044` with its `collected / contract` label), and the payoff bar. All numeric setup stats (max profit, score, P(profit)) and the leg breakdown live in **`SetupDetail`** (the detail panel), shown when the card is clicked (`onSelect` → `selectSetup(best)`). Dropping the stat row makes the strip shorter, reclaiming vertical space for the table.
  - **Props:** `ticker`, `best`, `count`, `isBest` (accent border + `best` badge), `compact` (drops under-bar zone labels), `onOpen`.
- **Per-ticker overview section** (in `App.jsx`, between the Holdings bar and `<main>`) — only renders after a scan returns results (`hasResult && overviewCards.length > 0`); nothing shows before the first scan. Header copy: `Best per ticker · {tickers_with_results} of {tickers_used.length} names had qualifying trades`. **Single-vs-multi rule:**
  - **One card** → a single **full-width** `OverviewCard` (`compact={false}`, full labels).
  - **Multiple cards** → **ONE horizontal strip** that scrolls sideways and **never wraps** to a second row: a `flex gap-3 overflow-x-auto no-scrollbar scroll-smooth` container whose children are fixed-width (`shrink-0 w-[260px]`) `compact` cards. Because it's a single row of fixed-height cards, the overview is a **fixed height regardless of ticker count** and never pushes the detail zone down. The container has right padding (`pr-10`) so the next card **peeks** at the right edge (obvious the row continues). Scroll affordances: native trackpad horizontal swipe; `onWheel` translates vertical wheel → horizontal `scrollLeft` for mouse users; and subtle ‹ / › **chevron buttons** (`scrollOverview(dir)` → `scrollBy`). The scrollbar is hidden via `.no-scrollbar` (defined in `index.css`) but stays scrollable.
  - The first card (highest score — `by_ticker` is pre-sorted by `best.score` desc) stays **leftmost** with the `best` **badge** (via `isBest`). The **accent border + ring** is separate (`selected` prop) and marks the **active card filter** (`cardFilter === ticker`) — so on a fresh scan no card is accented even though the rank-1 card still shows its badge. The section is `shrink-0`. Cards mirror the Holdings ticker filter (`overviewCards = byTicker.filter(...)`) so removing a pill hides the card. Each card's body click → `onSelect` → `toggleCardFilter(ticker)` (filters the table + detail to that ticker, or clears the filter on re-click). Cards have no inline stats or `legs ›` link — that all moved to `SetupDetail`.
- **`Holdings.jsx`** — dismissible "Scanning:" ticker pills shown after a scan; removing a pill instantly filters that ticker (table + overview). Renders as an **inline chips row** (no band wrapper) and is **folded into the controls band** as a full-width sub-row — it no longer occupies its own standalone band.
- **`MacroEvents.jsx`** — renders the upcoming macro events (FOMC, CPI, PPI, NFP) as a band of amber pills. **Collapsed by default** — App owns a `macroOpen` state (default `false`, **not persisted**, so it always loads collapsed) and renders `{macroOpen && <MacroEvents/>}` below the controls. A `macro ⌄ / ⌃` toggle button in the controls band flips it; when collapsed the whole band is absent (reference chrome, demoted out of the way).
...
- **`StockChart.jsx`** — three stacked Recharts `ComposedChart` panels (price candlesticks, volume bars, RSI line with 30/70 reference lines). It has two layout variants (`compact` fixed `h-[200px]`, `expanded` `flex-1`), but in the screener it is rendered as the **contextual chart panel in the detail zone** (right of the ranked table) using the `expanded` variant so it fills its panel. It is **no longer in the control bar**; after a scan the detail panel always shows a setup's chart (the rank-1 default, or the card-filtered / row-selected one) below `SetupDetail`. Its header has a `⤢ / ⤡` button (`onToggleFull` + `fullscreen` prop) that **expands the chart to fill the whole detail zone** (table hidden) and collapses it back to the side panel. (The optional `onClose` `×` is supported by the component but not wired in the screener, since the panel always has a default setup to show.)
...
- **`useChartData.js`** — fetches `/api/chart` for `(ticker, timeframe)`. **Lifted to App.jsx** (not used inside StockChart) so the contextual detail-zone chart panel gets its data from the App level. Caches by `ticker|timeframe` key in a `useRef(new Map())` and ignores out-of-order responses with a `latestKeyRef`.
