...
## Notes
- **Screener rebuilt on the v1 design system (2026-09-12).** `/app` was rebuilt from the approved shape (`.impeccable/surfaces/web-src-app-jsx.md`) in staged commits (a)–(g) plus a critique-fix round; see "Screener — rebuilt 2026-09-12" under Web UI. Impeccable critique: **17/40 → 29/40** (dual-agent re-run + verdict pass; detector 0 findings; snapshots in `.impeccable/critique/`); the four baseline P1s are closed (table explains its rank; rescans/errors keep the view; per-contract units everywhere with the boundary conversion; Save is a confirm-role button, idempotent per row). **Deployed: NO** — the user reviews locally first (local server: `python3 -c "from server.app import app; app.run(host='127.0.0.1', port=5002)"` → http://127.0.0.1:5002/app). Follow-ups recorded: sub-1100 layout (design problem, not a squeeze); Tradebook adopts the shell next; per-ticker earnings flags and per-ticker scan progress need `/api/run` additions; a server-side unique constraint on `(user_id, scan_id, result_id)` would make Save idempotent beyond the session; the "borderline" pill marks most rows because the strategy's P(max) sits structurally at 50–55% against the 50% default (product decision). Verification harness (gitignored): `.impeccable/review/_tools/` (app-shot, app-states, e2e-screener, app-leak).
