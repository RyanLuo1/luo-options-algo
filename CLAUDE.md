...
- **`lc/ControlsBar.jsx`** — one row of white cards: tickers / `@watchlist` (inline hint or error, Manage-watchlists toggle), weeks range (reuses `WeeksRangeSlider`), **"Minimum credit · $ per contract"** (default **$500** = the validated $5/share rule; App keeps `minPremium` per share for the API and converts at the boundary), minimum P(max profit), and **Run scan as the page's single lime action** (the One Action Rule). Enter in any field runs; ⌘/Ctrl+Enter runs from anywhere; `/` focuses tickers. Any control change after a scan shows "Rescan needed" with a violet ring.
- **`lc/ScanChips.jsx`** — real buttons per ticker with qualifying-setup counts; click filters to that ticker, × removes; skipped tickers explained in words.
- **`lc/RankedTable.jsx`** — the scanner's order by default (`rank` = position in `ranked`; filters never re-rank). A user column sort is an **override** with a visible "Back to ranked" affordance; App owns it (`sort`, `sortCtx`) so it persists across rescans of the same scan context (tickers + thresholds) and resets on a fresh one. Money columns sort descending first. Click or ↑/↓ select, Enter (or double-click) opens the editor. Columns: Rank · Ticker · Expires ("Oct 2 · W3 · 20d") · Put / Call / Call · Credit /ct · Max profit /ct with **`lc/MetricBar.jsx`** (a swappable metric: `metric` fn + `label` props — Screener passes `creditShareOfMax` from `lc/format.js`; Picks can pass ROC later with no re-layout) · P(max) · Collateral. Selected row = violet-soft fill + violet rank marker, no left border; rank 1 keeps the lime marker; borderline P(max) is a quiet pill, never a tint. 50 rows a page. Below 1280 the week fragment, the borderline pill and the Collateral column hide (all still in the panel).
