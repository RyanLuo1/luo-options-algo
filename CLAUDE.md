...
- **`lc/AppShell.jsx`** — the four-tab shell: wordmark, a real `tablist` (Screener · Tradebook · Picks 🔒 · Performance 🔒 — no Free/Paid pills on the tabs, the lock icon marks the paid ones; Tradebook navigates to `/tradebook`), market open/closed text badge with a short last-scan time, the plan pill (from `useAuth().plan`), Log out. `lc/LockedTeaser.jsx` renders the locked tabs' number-free teasers ("Paid · not available yet"; sample components only, no pricing).
