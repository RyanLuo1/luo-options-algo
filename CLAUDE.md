...
- **`Header.jsx`** — route-aware header (uses `useLocation`); rendered independently by each page component:
  - `/` (screener): a **contained surface panel** (`bg-surface`, rounded, `border-subtle`) laid out as **two zones via `justify-between`**: **left** = branding lockup (`Luo Capital / Options Screener`); **right** = the action cluster — market badge → **`⚙ Controls`** (toggles the left controls drawer; `onToggleControls` prop, accent border when `controlsOpen`) → Clear → **Run Scan** (the `bg-accent` primary, amber when stale) → Tradebook → a divided, muted `Last run {timestamp}` + `Log out`. The center is intentionally **empty space**. (Earlier iterations briefly put a TradingView ticker tape in the center — first hover-swapped with the controls, then always-visible — but it was removed as visual noise; the data that matters is in the cards/chart below.)
