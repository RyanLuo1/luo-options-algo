...
- **`lc/SetupPanel.jsx`** is one component in two states: the open dashboard (live spot marker, P(max) gauge, worst-case line) and, given `settlement` (an outcome row), the **graded** state — the settlement price becomes the curve marker ("settled 1040.00", collision-safe vs the max/breakeven labels), a **Realized P&L card** in the profit/loss pair replaces the gauge, and the worst-case slot becomes the settlement sentence (`settlementSentence` in `format.js`, one per zone). `expired` (grading-pending) shows "Expired …" pills, no dte, no spot, and an "Outcome: expired … grade posts after the next close" line. `actions`/`note`/`provenance`/`statusPill` slots let the Tradebook pass its own action row.
- **Actions:** Open in editor (**View in editor**, read-only, for graded trades), **Rerun this scan** (from the provenance pill; App.jsx accepts router state `{ rerun: { tickers, weeksMin, weeksMax, minPremium, minPProfit } }`, prefills the controls and runs once), a **two-step Delete** (Delete → Confirm delete / Keep; named toast). **No Clear all.** Provenance pill: "From your scan · Sep 12 22:13 · rank 1 of 553" from the user's own `scan_runs`/`scan_results` rows, or "Saved before scan history".
- **Editor (`pages/TradePage.jsx`)** re-skinned on tokens (same chains, same `/api/chain`, same transactable-side pricing, same `/api/tradebook/save`). Opened from the Tradebook with router state `{ triplet, scan_id, source: { id, status }, from: 'tradebook' }` the action is **"Save as new trade"** with **"Replace the original"** checked by default: on a successful insert the original row is deleted via supabase-js under the existing policy (two calls, no API change); unchecked → both rows remain and the caption says so. Graded trades open **read-only** (no chains, no Save). The summary strip therefore never counts an edit twice.
...
- **Tests:** `web/tests/e2e/tradebook.mjs` — save from the Screener → appears first as open → provenance + Rerun → edit with replace (open count unchanged, original gone) → edit without replace (both present) → delete with confirm → seeded graded + pending rows (service key; cleaned up) → summary → read-only editor → sort + keyboard → table fits at 1440. `seed-demo.mjs` creates a throwaway account with an open, a pending and two graded trades for a review (`--cleanup <user id>` removes it).
