...
- **`luo-capital-screener-state`** — App.jsx control state, written via a single `useEffect` whose deps include every persisted field. Persisted fields:
  ...
  - `cardFilter`, `selectedSetup` (detail-panel filter + row override; both `null` → panel shows the overall rank-1 default)
...
- **`App.jsx`** — screener page only (not a router/layout); owns all scan state, chart state, and control logic
  ...
  - The detail-zone chart is the **TradingView Advanced Chart embed** (`TradingViewChart.jsx`), driven by `toTvSymbol(chartTicker)` — App fetches no chart data from our backend. `cardFilter` and `selectedSetup` are persisted to sessionStorage.
