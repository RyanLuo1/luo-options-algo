...
- **`/api/run`:** `mode: "income" | "upside"` (default income; anything else is a 400). The Income path calls `scan_ticker` with no new kwargs (byte-identical to before). The Upside preset: leg B Δ 0.05–0.20, `min_premium` default $0.00 (a debit never qualifies), `min_upside` default 0.05 (a request parameter, 0–5). **The probability gate is off in Upside** (the client sends `min_p_profit: 0`; owner decision 2026-09-16 — "it's just a calculator"): the control is hidden in Upside and the gauge still shows every chance per setup. Income's gate and control are unchanged. Every ranked row carries `mode`; the response adds `mode_used`, `min_upside_used`, `leg_b_delta_used`; `ticker_reasons` gains `below_min_upside` and the code `min_upside`.
