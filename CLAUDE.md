...
- **Editor (`pages/TradePage.jsx`)** re-skinned on tokens (same chains, same `/api/chain`, same transactable-side pricing, same `/api/tradebook/save`). Opened from the Tradebook with router state `{ triplet, scan_id, source: { id, status }, from: 'tradebook' }` the action is **"Save as new trade"** with **"Replace the original"** checked by default: on a successful insert the original row is deleted via supabase-js under the existing policy (two calls, no API change); unchecked → both rows remain and the caption says so. Any non-open trade opens **read-only** (no chains, no Save). Each chain box scrolls the saved strike into its middle once the chain loads (`LegColumn`, once per load, not on clicks). The summary strip therefore never counts an edit twice.
