...
**Learned Ranker Roadmap (research, not product — nothing here becomes a user-facing surface; decided 2026-09-22)** — `docs/RANKER_SPEC.md` specifies the project in phases A–E: (A) Black-Scholes IV/delta module, (B) flat-file backtester (extract + replay), (C) descriptive analytics, (D) LightGBM ranker, (E) shadow mode. **Status (2026-09-12): A, B, and C are COMPLETE.** (A) gate passed 2026-07-26 (`lib/bs.py`). (B) the full year 2025-08-04 → 2026-07-25 was extracted by the B2 fleet, universe-gap re-streamed, and mirror-verified (complete 2026-09-02); the replay ran the v1 year (`backtest_open/close`, ~7.3k rows) and the v2 deep-slice year (`--v2` → `backtest2_open/close`, ~108k rows, dual-gate universe), both outcome-labeled split-aware. (C) findings + the corrected bake-off + v2 evaluation live in `docs/private/PHASE_C_FINDINGS.md` — headline: ROC ranking (return-on-collateral) beat the incumbent on the unbiased corpus (20.3 vs 10.6%/yr per collateral-day at slot-#1); the prespecified blend was REJECTED; the dual-gate universe and equal-collateral sizing are validated companion rules; **Phase D (ML) is NOT indicated on current evidence**. (E) **shadow mode is LIVE since 2026-09-08** (see the Phase E notes below and RANKER_SPEC §5c) — 4–8 weeks of live grading before any production ranking change. The evaluation standard everywhere: per-collateral-day annualized primary, tail veto, win rate diagnostic.
...
| `scripts/backfill_ml_outcomes.py`   | **Writes**  | After options expire — fills `ml_dataset` outcome columns (sector-scan setups; split-aware) |
| `scripts/view_outcomes.py`          | Read-only   | Any time — inspect / report on existing outcomes      |
| `scripts/audit_universe_coverage.py`| Writes doc  | Re-runnable — regenerates `docs/private/UNIVERSE_GAP_LEDGER.md` (per-extract scan-name coverage) |
| `scripts/phase_c_analytics.py`      | Writes doc  | Phase C — the eight descriptive cuts → `docs/private/PHASE_C_FINDINGS.md` tables |
| `scripts/phase_c_bakeoff_corrected.py` | Read-only | Corrected ROC-vs-incumbent bake-off (per-collateral-day, tail-first, guard sweeps) |
| `scripts/v2_evaluation.py`          | Read-only   | v2 corpus evaluation — true top-k, chance baseline, distrust triggers, universe effect |
...
```
A full scan is ~2.5–4.5 min sustained and **shares the Massive rate limit with the live app** (trigram.trade); 10:00 AM and 3:30 PM ET deliberately avoid the open/close volatility spikes.
...
- **Extraction universe = `data/universe_extract.json`** — the **$50B S&P superset + SPY/QQQ (240 names; rebuilt 2026-09-06** via `build_universe.py --threshold 50000000000 --out data/universe_extract.json --extra SPY QQQ --require-superset-of data/universe.json` after the 2026-08-07 build silently dropped 12 mega-caps — see the universe-gap runbook notes and `docs/private/UNIVERSE_GAP_LEDGER.md`), a superset of the scan's $100B universe so future point-in-time corrections are replay-side filters, not re-extractions. Tickers are matched as **OPRA roots** (`opra_root()`: `BRK-B` → `BRKB`), and `extract_day()` asserts extract-universe ⊇ scan-universe before streaming a byte.
...
### Screener — rebuilt 2026-09-12 on the v1 design system (`/app`)

The Screener page (`web/src/App.jsx`, route `/app`) was rebuilt from the approved shape in `.impeccable/surfaces/web-src-app-jsx.md` on the design system in `DESIGN.md` (the landing page's world). Components live under **`web/src/components/lc/`**; everything renders inside an element carrying the `.lc` class (index.css), which scopes the `lc-*` Tailwind tokens, the self-hosted Bricolage Grotesque / Figtree faces (`web/public/fonts`), tabular figures, selection, focus ring and scrollbar. The page is **light-only** and ignores the `dark` class on `<html>`; the unchanged WatchlistManager renders inside it via the `.lc-legacy` token override. **Sub-1100px widths are out of scope** (a recorded follow-up design problem); the layout is verified at 1440 and 1100.

- **`lc/AppShell.jsx`** — the shell: wordmark, a real `tablist` (**Screener · Tradebook**; Tradebook navigates to `/tradebook`), market open/closed text badge with a short last-scan time, Log out. **Product decision 2026-09-22: Picks and Performance are removed from the product** (the teaser component and its tab entries are deleted; `/picks` and `/performance` redirect to `/app` in `main.jsx`). The shell still accepts `plan` from `useAuth()` and renders nothing for it: the entitlement scaffold is dormant until the membership gate, which is built last (see PRODUCT.md → Information Architecture).
- **`lc/ControlsBar.jsx`** — one wrapping row of fields: tickers / `@watchlist` (inline hint or error, Manage-watchlists toggle), weeks range (reuses `WeeksRangeSlider`), the **dual-gate credit defaults** — **"Minimum credit"** (the `$` prefix and the ⓘ tooltip carry "per contract"; default **$100**, the friction floor: commissions + bid/ask slippage; App keeps `minPremium` per share for the API and divides by 100 at the boundary) and **"Minimum return on collateral"** (default **1%** = credit ÷ the cash the put ties up, `rocOf()` in `lc/format.js`; applied **live on the client** for instant feedback **and, since 2026-09-24, on the server** (DEPLOYED 2026-09-24, commit 418542e on the box; `scan_runs.min_roc` DDL applied by the owner): Income requests send `min_roc`, `scan_ticker(..., min_roc=0.0)` gates on credit ÷ put strike after the probability gate — off by default, so the default path, the cron, the replay and the CLI are byte-identical (golden test asserts the callers pass nothing) — and the run is logged **after** the floor, so `scan_results` holds exactly the rows the user saw and the Tradebook's provenance pill "rank N of M" equals the table's rank and count at scan time; `scan_runs.min_roc` records the floor (`docs/scan_roc_migration.sql`, written only when set, insert retried without it until the DDL runs); the response carries `min_roc_used`; a ticker whose candidates cleared the credit rule but not the floor reads "N candidates cleared $X but not the 1% return floor" (`min_roc`). Changing the floor between scans still re-filters live; the logged run keeps the floor it was scanned under) — minimum P(max profit), and **Run scan as the page's single lime action** (the One Action Rule). **Corpus isolation:** user web scans log to `scan_runs` / `scan_results` (per-user, RLS) and never to `ml_dataset`; the research corpus is written only by `scripts/sector_scan.py` / `replay_scan.py` / `backfill_ml_outcomes.py`, whose validated **$5/share** rule is unchanged. Enter in any field runs; ⌘/Ctrl+Enter runs from anywhere; `/` focuses tickers. Any control change after a scan shows "Rescan needed" with a violet ring.
- **`lc/ScanChips.jsx`** — two lines (owner, 2026-09-18): "Scanning" holds the tickers that produced setups, together; "Didn't make the cut" sits under it with the zero tickers one per line, each with its cause and, in Upside, the thin-quote link. Real buttons per ticker with qualifying-setup counts; click filters to that ticker, × removes; a ticker that yields zero carries its cause in words ("NVDA — no setup cleared the $100 minimum" / "…the 1% return floor" / "liquidity guards…" / "no options chain returned"). The cause comes from **`ticker_reasons`** in the `/api/run` response — the minimal API addition: `screener.scan_ticker()` gained an optional `stats=` dict of rejection counters (`no_chain`, `no_legs`, `below_min_premium`, `below_min_p`; bookkeeping only, no filter or score change) and `server/app.py` maps them via `_zero_reason()` to `no_chain | min_credit | min_p | min_upside | min_credit_or_p`, or — when no triplet was ever evaluated — to the exact liquidity cause from the **per-ticker liquidity census** (2026-09-17, commit one of the liquidity brief in `.impeccable/surfaces/web-src-app-jsx.md`): `_parse_massive_contracts(raw, census=None)` tallies each contract under the first guard it fails (`CENSUS_KEYS`: contracts → no_greeks / placeholder_iv / no_quote / wide_spread / untraded → tradeable; bookkeeping only, the default path is byte-identical and the golden test proves it; the census rides on the live provider via `scan_ticker(..., census=)`, so cron/replay/CLI never carry one). Codes `wide_spread | no_quote | placeholder_iv | untraded` name the guard that killed the most contracts when none survived; `no_triplet` means legs survived but never all three on one expiration. The chip line reads "quotes too wide" / "no live quotes" / "untraded" / "no complete three-leg setup"; the hover carries the counts (`zeroReasonDetail` in `lc/format.js`: "1,512 contracts seen: 552 quotes too wide · 196 no live quote · 677 untraded · 87 tradeable") for every zero chip, whatever its cause. `ticker_reasons[t].census` is in the `/api/run` response. The client adds `roc` when the API returned rows that the return floor removed.
- **The liquidity ladder — Tier 1 only, display-only, Upside-only (2026-09-17, commit two of the liquidity brief; the owner chose the smaller build). DEPLOYED 2026-09-22 (commit c0b42dc on the box; golden test green there; live API returns the ladder and rejects `relax` in Income).** Tier 0 = today's four guards, unchanged. Tier 1 = the volume floor off; the two-sided quote, the 15% spread cap and the user's credit / probability / upside floors all hold. Tier 2 (spread cap 30 → 50%) and saving relaxed rows are **not built** — deferred until Tier 1 shows use. Mechanics: in Upside, `/api/run` scans through `screener.CachedLiveChains` (each (ticker, expiration, side) snapshot fetched once; `provider(min_volume=MIN_VOLUME)` is the Tier 0 parse, identical contracts to the live provider) and, for a ticker that produced nothing at Tier 0, re-runs `scan_ticker` with `provider(min_volume=0)` from the chains already in memory — **zero extra Massive calls** (`_tier1_rows`). Income never touches the cache (its call is the pre-mode call). Response: every ranked row carries `tier: 0`; Upside responses add `relaxed[ticker] = {tier: 1, rows, evaluated, reason}` (rows stamped `tier: 1`, sorted by max profit ÷ collateral, each leg with `leg_x_mid` / `leg_x_volume`) and `ladder` (the two tiers' floors). **The ladder is fixed:** a `tier` key in any request is a 400; `relax` in an Income request is a 400 (Upside may pass `relax: false` to skip the ladder). **Relaxed rows never enter the data:** they are not in `ranked`, never logged to `scan_results` (`log_scan_run` sees only the Tier 0 list), and `/api/tradebook/save` rejects a trade carrying `tier ≥ 1` with a 400; the research writers never call this path. UI: a zero chip in Upside offers "Show thin-quote setups (worst-case priced)" when Tier 1 found rows (the hover says "none with the volume floor off either" otherwise); clicking swaps the results zone to **`lc/RelaxedGroup.jsx`** — one ticker's thin-quote setups shown **in place of** the ranked list (owner, 2026-09-18: one view at a time, no scroll; the chip's link becomes "Back to ranked setups" and the group's head carries the same button; the ranked table keeps its sort, selection and grouping across the round trip) — its own labelled section with a "Volume floor off" pill (**tiers are internal**: `tier` lives in the API and `data-tier` attributes, never in user-facing copy — owner, 2026-09-18), the ranked list's grammar (the best row by max profit ÷ collateral, then "+N more MU setups" / → ←), the line naming what still holds, the **liquidity-cost headline** for the selected row (`liquidityCost` in `lc/format.js`: worst case = the credit shown; at the mid = the same legs at each quote's mid; gap = what patience is worth, with the ⓘ saying a limit order inside the spread recovers some of it, never guaranteed), the rows (credit at the bid/ask / gap / max profit / max profit per $ of collateral / shorts worthless), and the dashboard panel — whose leg tiles wear a dashed violet edge on each leg that came in below the volume floor and, on hover or focus, a popover with today's volume and open interest (`Leg` in `lc/SetupPanel.jsx`, from `leg_x_volume` / `leg_x_oi`, so only thin-quote rows carry it; the parser now keeps `oi` on each contract for display — no filter reads it; owner, 2026-09-18: no pills) — with a "Volume floor off" pill, **View in editor** (read-only in `TradePage.jsx`, `source.status === 'relaxed'`) and no Save. The Tier 0 table is keyed on the scan context and never sees relaxed rows (byte-identical before and after the round trip); the view and the group's selected row are persisted with the scan context (so a trip to the editor comes back to them) and reset with a new scan. Tests: `web/tests/e2e/liquidity.mjs` (the four commitment tests + the flow). Tonight's probes: GEV's zero is the credit floor (Tier 1 changes nothing), APP's is the spread cap (Tier 2 territory, not built), MU weeks 7–8 and META weeks 7–9 are the Tier 1 cases.
- **`lc/RankedTable.jsx`** — **grouped by default ("Best per ticker")**: each ticker's best setup is one row in algorithm order across tickers, a "+N more" expander reveals that ticker's variants in algorithm order, → / ← expand or collapse from the keyboard, and a "Flat list" toggle (persisted as `grouped`) shows every row. Sorting, selection, keyboard nav and "Back to ranked" work in both modes (a sort reorders groups by their best row and variants within). `rank` = position in the scanner's order **after the client-side return floor** (`rocRanked` in App); chip filters never re-rank; the component is keyed on the scan context so paging and expansion reset with a fresh scan. **No borderline pill** (P(max) is shown; the pill marked nearly every row). A user column sort is an **override** with a visible "Back to ranked" affordance; App owns it (`sort`, `sortCtx`) so it persists across rescans of the same scan context (tickers + thresholds) and resets on a fresh one. Money columns sort descending first. Click or ↑/↓ select, Enter (or double-click) opens the editor. Columns: Rank · Ticker · Expires ("Oct 2 · W3 · 20d") · Put / Call / Call · Credit /ct · Max profit /ct with **`lc/MetricBar.jsx`** (a swappable metric: `metric` fn + `label` props — Screener passes `creditShareOfMax` from `lc/format.js`; Upside passes max profit ÷ collateral with no re-layout) · P(max) · Collateral. Selected row = violet-soft fill + violet rank marker, no left border; rank 1 keeps the lime marker. 50 rows a page. Below 1280 the week fragment, the borderline pill and the Collateral column hide (all still in the panel).
- **`lc/SetupPanel.jsx`** — the detail panel driven by the selection: ticker + spot, expiry/week/DTE pills, macro events dated before expiry (scan-wide from `macro_events`, so they live here, not per row; per-ticker earnings flags are not in the API row shape — follow-up), three leg tiles on the pay/collect convention, **`lc/PayoffCurve.jsx`** (zones labelled in words, spot marker, collision-safe labels, "spot unavailable" degrade), stat cards (credit, max profit, P(max) gauge, collateral, breakeven), the worst-case line, **Save to Tradebook** as the ink "confirm" role (disabled while saving → no double insert; errors in-panel) and Open in editor. The toast names the saved trade and links to `/tradebook`.
- **`lc/States.jsx`** — the non-happy states in words: `ProgressStrip` (rescans keep the table and panel mounted at 60% under an honest indeterminate strip "Scanning N tickers · Ns"; per-ticker progress needs an API change), `ErrorStrip` (dismissable, problem + recovery, never a port number, results kept), `MarketClosedBanner`, `NoResults` (cause-specific: every ticker skipped vs thresholds too strict, with actions that lower a threshold and rerun via App's `runWith(overrides)`).
- **`lc/ui.jsx`** — `Button` (primary lime / secondary / ghost / **confirm** ink), `Pill` (violet / quiet / lime / profit / loss), `Card`, `Field`, `Input`, inline SVG icons. **`lc/format.js`** — formatters, `expiryInfo`, `rowFigures` (per-contract figures), `rowKey`, `creditShareOfMax`, `TABS`, `INPUT_CLASS`.
- **State:** App persists `tickerInput, activeTickers, weeksMin/Max, minPremium, minCreditStr, minPProfit, minPProfitStr, tickerFilter, sort, sortCtx, selectedKey` to sessionStorage (same `sessionState.js` keys). New scan results are handled with React's adjust-state-during-render pattern (no set-state-in-effect). The old `Header.jsx` screener variant, `ControlsDrawer`, `Holdings`, `ResultsTable`, `SetupDetail`, `LoadingSpinner` and `TradingViewChart` are no longer used by the Screener (`Header.jsx`, `Toast.jsx` and `ThemeToggle.jsx` were deleted 2026-09-24 as orphans with no importers; every page renders its own toast markup inside the shell). **There is no price chart on the Screener anymore.**
- **Tests — `web/tests/e2e/`** (tracked; plain Node + Playwright, no runner): `npm run test:e2e -- <baseUrl>` runs `states.mjs` (defaults, grouping, live return floor, loading/error/no-results, keyboard, sort, idempotent Save, filter-empty), `flow.mjs` (fresh account → watchlist → scan → sort → select → save → toast → Tradebook → DB provenance) and `shots.mjs` (1440/1100 captures with overflow + font checks). Each creates and deletes a throwaway account via the sign-up API (`../.env` service key, `web/.env` anon key); `E2E_EMAIL`/`E2E_PASSWORD` use an existing account. Screenshots go to `web/tests/e2e/out/` (gitignored). See `web/tests/e2e/README.md`.
- **Verification (2026-09-12):** Playwright end-to-end (fresh account → create `@e2e` watchlist → scan → sort → select → save → toast → Tradebook shows it, DB row with `scan_id`/`result_id`), scripted state checks, no overflow at 1440/1100, lint clean; impeccable critique re-run — see the Notes entry.

### Tradebook — rebuilt 2026-09-12 on the v1 design system (`/tradebook`) — DEPLOYED 2026-09-13

The Tradebook (`web/src/pages/TradebookPage.jsx`) was rebuilt from the approved shape in `.impeccable/surfaces/web-src-pages-tradebookpage-jsx.md` on the same shell and tokens as the Screener. **Logic unchanged:** reads are direct supabase-js queries under the existing row-level policies, saves still go through `/api/tradebook/save`, delete is the existing per-row delete; the tradebook API, schema, backfill and grading logic were not touched. Mobile is out of scope (≥1100 is the floor).

- **`hooks/useTradebook.js`** — the page's data: the user's `tradebook` rows joined client-side with `trade_outcomes` (by `tradebook_id`), `scan_runs` (by `scan_id`) and `scan_results.rank` (by `result_id`). Status per row: **open** (expiration ≥ today ET), **pending** (expired, no outcome row yet — the nightly backfill hasn't graded it), **graded** (outcome row exists). Outcome/provenance reads are best-effort so a policy hiccup never hides the book. `summary` = open (pending included), graded, total `pnl_per_contract`. `fetchSpot(ticker)` = live spot for open trades via the existing `/api/chart` (cached 60 s).
- **Summary strip** — three figures: Open ("N open · M grading" when pending exist), Graded, Total realized P&L (signed, profit/loss ink) captioned exactly *"sum of per-contract outcomes, not a portfolio return"*.
- **`lc/TradebookTable.jsx`** — the Screener's row grammar: Status pill (Open violet / Grading pending quiet / Graded **lime**, the theme's yellow, as the landing's "graded" chips — owner, 2026-09-22) · Ticker · Expires ("Oct 2 · 20d" / "Sep 11 · expired" / "Aug 21 · settled") · Put / Call / Call · Credit /ct · Collateral · P&L /ct (signed; "—" open, "pending" while grading) · Zone (short label) · Saved (date). Collateral, Zone and Saved hide ≤1100. **Default order:** grading-pending first, then open nearest-expiration-first, then graded newest-first (`defaultTradeOrder` in `lc/format.js`); column sorts are overrides with "Back to default"; ↑/↓ select, Enter opens the editor; `data-status` on each row.
- **`lc/SetupPanel.jsx`** is one component in three shapes. **Open:** the dashboard (live spot marker, P(max) gauge, worst-case line). **Graded** (given `settlement`, an outcome row): a *different shape* — the **Realized P&L card** (profit/loss pair, signed, 2.2rem) and the **settlement sentence** (`settlementSentence` in `format.js`, one per zone) lead directly under the head, and "The setup as saved" (legs, the curve with the settled-price marker, credit / max / collateral / breakeven in past tense) is a native `<details>` fold, collapsed by default, whose summary carries the strikes, credit and max. **Expired but ungraded** (`expired`): the open shape with "Expired …" pills, no dte, no spot, a neutral **"Grade pending · posts after the next close"** tile in the gauge slot, and an "Outcome: expired …" line. `actions`/`note`/`provenance`/`statusPill` slots let the Tradebook pass its own action row; with `stickyActions` that row pins to the bottom edge of the panel's scroll container. On the Tradebook the panel column is `sticky top-4` and scrolls on its own — its max-height is set from the viewport space below its current top (a scroll/resize listener in `TradebookPage.jsx`), so Open in editor / Delete are always visible without scrolling the page.
- **Actions:** Open in editor (**View in editor**, read-only, for any trade that is not open — graded *or* grading-pending, so a replace can never delete the row the nightly backfill is about to grade), **Rerun this scan** (from the provenance pill; App.jsx accepts router state `{ rerun: { tickers, weeksMin, weeksMax, minPremium, minPProfit } }`, prefills the controls and runs once), a **two-step Delete** (Delete → Confirm delete / Keep; named toast). **No Clear all.** Provenance pill: "From your scan · Sep 12 22:13 · rank 1 of 553" from the user's own `scan_runs`/`scan_results` rows, or "Saved before scan history".
- **Editor (`pages/TradePage.jsx`)** re-skinned on tokens (same chains, same `/api/chain`, same transactable-side pricing, same `/api/tradebook/save`). Its summary strip shows Credit /ct (or Debit /ct), Max profit /ct, Spread width and **the gauge triad** — Shorts worthless (1 − δ_B − δ_C), Max profit ≈ δ_B, Put assigned ≈ δ_C — from the selected legs' deltas, updated on Recalculate (owner, 2026-09-17: P(max) is no longer shown anywhere; `p_max_profit` is still computed and saved). Opened from the Tradebook with router state `{ triplet, scan_id, source: { id, status }, from: 'tradebook' }` the action is **"Save as new trade"** with **"Replace the original"** checked by default: on a successful insert the original row is deleted via supabase-js under the existing policy (two calls, no API change); unchecked → both rows remain and the caption says so. Any non-open trade opens **read-only** (no chains, no Save). Each chain box scrolls the saved strike into its middle once the chain loads (`LegColumn`, once per load, not on clicks). The editor opens at the top of the page, and **Back to Tradebook** hands the trade's id back as router state `{ selected }` so the Tradebook reselects it (one-shot: the page clears the state on mount so a reload starts from the default row). The summary strip therefore never counts an edit twice.
- **States:** empty (Go to the Screener), loading strip, error strip (Tradebook wording, retry/dismiss), grading-pending ("Grading pending · grades after the next close").
- **Tests:** `web/tests/e2e/tradebook.mjs` — save from the Screener → appears first as open → provenance + Rerun → edit with replace (open count unchanged, original gone; chains open with the saved strike in view) → edit without replace (both present) → delete with confirm → seeded graded + pending rows (service key; cleaned up) → pending panel (Grade-pending tile, read-only editor) → graded panel (answer above the fold at 1440×900, setup folded, settled marker inside the fold) → panel actions visible without scrolling → Back returns to the trade you came from → summary → sort + keyboard → table fits at 1440. `seed-demo.mjs` creates a throwaway account with an open, a pending and two graded trades for a review (`--cleanup <user id>` removes it).

### Screener modes — Income · Upside (2026-09-15) — DEPLOYED 2026-09-16 (commit 894e655 on the box; golden test green there; all six e2e scripts green live)

**Upside is the same three legs built for a wide call spread and a small credit.** (The "Not backtested — calculator only" tag was retired 2026-09-16 by the owner as the Upside backtest neared completion; Upside rows carry a quiet "Upside" pill.) Brief: `.impeccable/surfaces/web-src-app-jsx.md` § Upside mode (local only).

- **Backend (default-preserving):** `screener.scan_ticker(..., leg_b_delta=None, min_upside=0.0)` — the short-call delta window (default `(LEG_B_DELTA_LOW, LEG_B_DELTA_HIGH)`) and a floor on max profit ÷ collateral per share (`(net_premium + spread_width) / leg_c_strike`; 0 = off, and the `below_min_upside` counter exists only when it is on). The gate order is premium → width → P → upside. **`tests/test_mode_defaults.py` holds a golden fixture recorded from the pre-parameter scanner**: the default call path must reproduce it byte for byte (content, order, and the `stats` dict), an explicit-defaults call must equal the default call, and the cron / replay / CLI call sites must pass neither keyword. Re-record only deliberately (`python3 tests/test_mode_defaults.py --record`, reason in the commit).
- **`/api/run`:** `mode: "income" | "upside"` (default income; anything else is a 400). The Income path calls `scan_ticker` with no new kwargs (byte-identical to before). The Upside preset: leg B Δ 0.05–0.20, `min_premium` default $0.00, `min_upside` default 0.05 (a request parameter, 0–5). **Upside accepts a negative `min_premium`** (owner decision 2026-09-17: a calculator should let you look at net-debit structures; **DEPLOYED 2026-09-17, commit f0bdfae on the box**) — the server validates Income's floor as ≥ 0 and Upside's as any finite number; the client's credit field goes negative only in Upside (Income shows "Enter 0 or more."). A debit row renders honestly: the credit cell in loss ink as "−$120", the panel's stat reads **Debit paid** "per contract, paid to open — the flat zone is a loss", the payoff curve labels the flat zone "flat loss" and "−$120 debit", and the breakeven follows the shape (`rowFigures` in `lc/format.js`: a credit breaks even below the put at K_C − credit; a debit breaks even above the long call at K_A + debit, or never when max profit ≤ 0 — "none"). The worst-case sentence uses the separate `effectiveCost` (K_C − net premium) so an assigned put's cost basis is right in both cases. Grading is unchanged: a debit that expires below the long call is `expired_loss` by the existing rule. **The probability gate is a strategy term, not a user control** (owner decision 2026-09-16): no mode exposes it. Income requests carry it unchanged as a fixed `min_p_profit: 0.5` (`MIN_P_PROFIT` in `App.jsx`; the scanner's default path and the golden test are untouched); Upside sends `0` (a calculator). The bar is four fields plus Run scan in both modes; the gauge triad is the probability view. Every ranked row carries `mode`; the response adds `mode_used`, `min_upside_used`, `leg_b_delta_used`; `ticker_reasons` gains `below_min_upside` and the code `min_upside`.
- **Logging and the ledger:** `scan_runs.mode` and `tradebook.mode` (`docs/scan_mode_migration.sql`, **applied by the owner 2026-09-16**; Upside scans log with provenance and Upside saves land labelled). The server writes the column **only for upside rows**, so income inserts are unchanged whether or not the DDL has run; an upside save before the DDL fails loudly rather than landing as an income trade. `ml_dataset`, `sector_scan.py`, `replay_scan.py` and the backfills are untouched; web scans stay out of the corpus.
- **UI:** a two-segment mode control at the head of the controls bar with two-line help per mode ("Get paid to wait." / "Own the upside."). Switching swaps only the gate field: Income keeps Minimum credit ($100) + Minimum return on collateral (1%, live); Upside shows Minimum credit ($0) + **Minimum upside per $ of collateral** (5%, rescan to apply). No probability control in either mode. Each mode keeps its own credit floor and its own last result (`useOptionsData(mode)` stores results per mode; sessionStorage holds `{ results }`). Upside: the metric bar = max profit ÷ collateral (a prop: `metric` / `metricLabel` on `RankedTable`), a **Move to max** column ((K_B − spot) / spot, descriptive), the Max-profit card highlighted instead of Credit. **Upside is a sorted list, not a ranking** (critique fix 2026-09-16): heading "Upside setups", status "Sorted by max profit per $ of collateral" — its default order, applied **server-side** in `/api/run` for `mode: upside` since 2026-09-22, **deployed 2026-09-24 (commit 6541d19 on the box)** (research review: the incumbent score is ~0 for wide setups, so ordering Upside by it was noise; the API's `ranked`, the logged `scan_results.rank` and the table now agree; the client applies the same sort defensively in `App.jsx`); Income keeps the scanner's score order, dot rank cells, never the lime rank-1 marker. **The table never mixes modes** (owner decision 2026-09-16: the two-head-row side-by-side was built, then removed — the mode toggle is the side-by-side, each mode showing its own last result). Never "pick" in copy (it reads as a recommendation). The ⓘ tooltips (`InfoTip` in `lc/ui.jsx`) open leftward near the viewport's right edge; the P control's helper reads "Gates ~5 pts above the chance shown." The gauge is two labelled figures across both stat columns. A failed save renders a plain sentence (the server maps the missing-`mode`-column case to "The Tradebook can't store Upside trades until its mode column exists…"; the client never renders a message containing braces).
- **The gauge (confirmed 2026-09-15):** `p_max_profit = (1 − δ_B)(1 − δ_C)` is neither the chance of max profit nor exactly the chance both shorts expire worthless; that chance is 1 − δ_B − δ_C (the product overstates it by δ_B·δ_C, 3–12 points at the default windows), and max profit needs the stock at or above the short call, ≈ δ_B. The UI now shows **"Chance the short legs expire worthless"** = 1 − δ_B − δ_C (`shortsWorthless` in `lc/format.js`; gauge, the table's "Shorts worthless" column, the landing card) with the helper "the stock finishes between the put and the short call — you keep at least the credit", plus **"Chance of max profit ≈ δ_B"**. The scanner's 50% gate keeps the product; its control is "Minimum chance shorts expire worthless (approx.)" with the ⓘ explaining the gap. `p_max_profit` is still computed, gated, stored and logged exactly as before.
- **One collateral definition everywhere** (DESIGN.md copy conventions): the put strike × 100, `collateralOf` in `lc/format.js`.
- Tests: `tests/test_mode_defaults.py`; `web/tests/e2e/modes.mjs` (needs the two `mode` columns for the Upside-save round-trip).

> The three sections below ("Single-strategy screener", "Layout — viewport-locked…", and the screener parts of "Key components") describe the **previous** screener and are retained for the trade editor / tradebook / login pages they still document. Where they contradict the section above, the section above wins.
...
- **`web/src/pages/LoginPage.jsx`** — rebuilt 2026-09-13 on the v1 design system, **DEPLOYED 2026-09-13 (commit 897049d on the box; live `auth.mjs` green including a real recovery link, so the production redirect URL is configured)** (brief: `.impeccable/surfaces/web-src-pages-loginpage-jsx.md`, local only). **One card, four states**, on the landing's ground with the wordmark in the app shell's position ("turning a page"): **Log in · Create account** (a `tablist` mode control that doubles as the title; shared Email + Password fields, shared client validation, shared error strip; the typed email survives a mode switch), **Reset your password** (email only → `resetPasswordForEmail(email, { redirectTo: origin + '/login' })`), and **Set a new password** (the recovery link lands on `/login` with `#…type=recovery`; supabase-js emits `PASSWORD_RECOVERY`; `updateUser({ password })` → `/app`). The hash is read at module evaluation (supabase-js strips it asynchronously) and only honoured on the initial navigation entry (`location.key === 'default'`), so logging out and returning to `/login` never reopens the recovery state. The usual logged-in → redirect is held while recovering or while an expired-link message (`#error_code=otp_expired…`) is on screen. **Return-to:** `ProtectedRoute` bounces with `state={{ from }}`; login navigates back there (only `/app`, `/trade`, `/tradebook`), default `/app`. `/login?mode=signup` opens in Create account. Light-only (the old ThemeToggle / split-screen candle panel are gone; the whole theme path — `ThemeScope`, `useTheme.js`, the FOUC guard and the `dark` class in `index.html` — was removed 2026-09-24: the product is light-only and `.lc` never read the class).
  - **Error register** (`describe()` — never a Supabase message verbatim): bad login → "That email and password don't match. Check the password, or create an account if you don't have one yet." (+ Create account; Supabase returns one `invalid_credentials` for wrong password and unknown email by design); account exists → "An account already exists for {email}." (+ Log in instead; also Supabase's no-identities reply); weak password → "Passwords need at least 6 characters." (Supabase's configured rule, length only — also checked client-side); bad email shape; unconfirmed email (notice tone; inactive with auto-confirm on); rate limit; network failure (+ Try again); expired reset link (+ Request a new one); `same_password`.
  - **Mobile is in scope here:** 16px gutters, the card fills the width at 390, 48px inputs at 16px type (no iOS zoom), Show/Hide inside the field with a 44px hit area, `autocomplete` email / current-password / new-password. One authored motion: the card rises 8px and fades in (`.lc-card-in`, reduced-motion honoured). Sign-up's only claim: "Free account. The screener and tradebook are yours — no card, no trial clock."
  - **Supabase setup for the reset flow (owner, one time):** Authentication → URL Configuration → Site URL `https://trigram.trade`; Redirect URLs must include `https://trigram.trade/login` (and `http://127.0.0.1:5002/login` for local verification — this one already worked on 2026-09-13). Without the production entry the emailed link lands on the Site URL instead of the card.
  - Tests: `web/tests/e2e/auth.mjs` (see the e2e README) — includes a REAL recovery link via the admin `generate_link` API.
- **`ProtectedRoute`** (in `main.jsx`) — wraps routes requiring auth; redirects to `/login` if `user` is null after loading, passing the bounced route as router state `{ from: pathname + search }` so login can return there
...
**Unlisted until public launch (launch-day removals, remove together):** Flask serves `/robots.txt` as `User-agent: * / Disallow: /` (`robots_txt()` in `server/app.py`), and the landing page carries `<meta name="robots" content="noindex, nofollow">` (`design/landing/v1/index.html`, just above the viewport meta). The public-launch gate checklist lives in `PRODUCT.md` → Users.

**Entitlements (Phase 1 scaffold, 2026-09-12; DORMANT since 2026-09-22 — nothing is gated, no plan is shown; kept for the flat-membership gate, built last):** a user's plan lives in the Supabase auth user's **`app_metadata.plan`** (`'free'` | `'paid'`). Absence means free, so every new sign-up is free with no trigger or table. Only the service role can write `app_metadata` (admin API `PUT /auth/v1/admin/users/{id}` with `{"app_metadata":{"plan":"paid"}}`), so the claim in the JWT is unforgeable; RLS can read it as `auth.jwt() -> 'app_metadata' ->> 'plan'` when paid tables arrive. Frontend: `web/src/lib/entitlements.js` (`getPlan(user)`, `isPaid(user)`) and `useAuth()` returns `plan`. Backend: `user_plan(user)` in `server/app.py` next to `verify_token`. A `profiles` table was deliberately NOT added: it would be a second source of truth, and DDL cannot be run from the dev machine (no CLI/psql/DB URL). Upgrade to a table only if plan needs history or admin UI.
...
- **`pages/TradePage.jsx`** / **`pages/TradebookPage.jsx`** — both rebuilt on the v1 design system 2026-09-12; see "Tradebook — rebuilt 2026-09-12" above (the old dark-token table with Date Saved / Score / P(Profit)% columns and the Clear-all button are gone).
...
**B2 year-stream fleet — COMPLETE AND DECOMMISSIONED (2026-09).** The claims-coordinated fleet (`scripts/b2_worker.py`, c7i box) streamed the full year 2025-08-04 → 2026-07-25, absorbed the universe-gap re-stream pass (147 dates re-extracted with the fixed 240-name universe, finished 2026-09-02), and passed the final assertion sweep. **The termination precondition was SATISFIED 2026-09-02** (mirror byte-verified against the manifest, full range, both eras; currently 278/278) and the box has been unreachable since ~2026-09-03 — terminated via the owner console (if it was in fact only stopped, it's idle cost: confirm in the console). The Mac mirror in `data/extracts/` (+ archives `pre_restream_originals/`) is now the canonical copy alongside the manifest; the t3 continues the daily forward drip. The runbook below is retained as the record of how completion was verified.

**Fleet completion runbook** (run in order when the fleet range finishes): (1) **final-validation sweep — ASSERT completeness from the claims table, never assume it**: every trading day of 2025-08-04 → 2026-07-25 `done`; **zero** non-holiday `failed` rows (market-holiday NoSuchKey rows are the only tolerated failures — verify each against the NYSE calendar); **zero** stale or in-flight `claimed` rows; every `done` row carries `parquet_bytes`; and every OOM/retirement casualty (e.g. 2026-01-30's two-death re-stream, any retire-one orphaned claim) explicitly re-verified `done` — a date that died mid-stream and was never re-claimed is exactly what this assertion exists to catch; (2) Mac mirror verification against the manifest — file count AND per-file sizes matching `parquet_bytes` for the FULL range; (2.5) if the t3 catch-up backlog persists at fleet completion, point the idle c7i at the remaining live catch-up dates before termination — an afternoon of fleet-speed catch-up vs days of the t3 treading water at ~1 file/day; (2.6) **confirm the universe-gap re-stream scope before terminating** (decision filed 2026-08-27, confirms at completion, informed by `docs/private/UNIVERSE_GAP_LEDGER.md`) — current lean: **hybrid (c)** — the year-replay runs at 106 names on schedule; the 12-name re-stream runs as a background second pass on the existing fleet post-completion (~9–11 days, ~$80–95 at current pace); a delta-replay adds the recovered names' rows afterward (slot-split `source` semantics make per-source re-runs safe); (3) only after (2) verifies clean (and 2.5/2.6 as applicable), terminate the box.

**Completion-day execution checklist (staged 2026-08-28, ~48–72h out)** — the runbook concretized, in firing order:
- **(a) Final sweep** with the step-1 assertions (claims-table completeness, zero non-holiday failures, zero stale claims, casualties re-verified). One re-run of `scripts/audit_universe_coverage.py` for the final ledger.
- **(b) Mirror go/no-go artifact**: full-range verification — file count AND per-file `parquet_bytes`, both eras. This is the termination gate; its output gets pasted into the decision record.
- **(c) Re-stream scope confirmation (step 2.6)**: pull final numbers from `docs/private/UNIVERSE_GAP_LEDGER.md` (12-name dates by era, BRKB dates, the ~7 gapped in-flight dates from the Aug-27 fix window, any casualties) and present the decision package: dates, projected c7i-days **at measured current speeds** (2026-08-28 measurement: ~41 files/day ≈ 5.5–6 TB/day → the ~155–160-date / ~21 TB scope runs **~3.5–4.5 c7i-days, ~$30–40** — down from the 9–11-day/$80–95 quote at degraded speeds), cost. **Mechanical prep note**: `done` claims are terminal and extracts are immutable-skip — the re-stream pass needs scoped prep (move the gapped parquets aside + reset their claims rows to `failed`) before workers will touch those dates.
- **(d) c7i termination** — owner's console action, gated on (b) AND the (c) decision: if re-stream is GO, the box lives until the re-stream pass completes and re-verifies through (a)+(b) again.
- **⚡ (c) DECIDED: GO (owner, 2026-08-29, pre-travel) — and automated.** `~/restream_orchestrator.sh` on the fleet box (launched 2026-08-29 16:17Z, nohup) waits for the completion gate (`~/restream_prep.py --gate-only`: 245/245 fleet-range done, failures ⊆ market holidays, zero in-flight claims; self-heals dead workers with a throttled mop-up relaunch), then runs prep `--execute` (moves gapped parquets to `data/extracts/gapped/`, resets scoped claims — done rows ≤ 2026-08-21 claimed before 2026-08-27T23:15Z — to `failed`) and launches **6 re-stream workers `--start 2025-08-04 --end 2026-08-21`**. Halts loudly without launching if prep fails. The t3 catch-up can't interfere (it skips dates whose local parquet exists before touching claims). day_aggs are reused in place. Termination gate (d) unchanged — owner console action only, after the re-stream re-verifies via (a)+(b).
- **(e) Year-replay launch, ready to fire**: `scripts/replay_scan.py --write --slot both --top-n 10 --spot-source implied` with repeatable `--date` args, **chunked by month**, resumable against the de-dup keys (re-running a (source, date, sector) slot replaces cleanly). Pre-flight: confirm the PIT macro schedules (`data/macro_schedules/2025.json`/`2026.json`) satisfy the >14-day-back guard, and run one dry-run month first (no `--write`).
...
- **Domain:** https://trigram.trade (primary since 2026-09-22; registered via Namecheap, DNS A records for @ and www pointing to the Elastic IP). The old https://luo-capital.com 301s here permanently (own cert, kept valid). **Naming record: the product is “Trigram”; “Luo Capital” is the owning entity (maker's mark: “Trigram by Luo Capital”). GitHub repo name, systemd service (`luocapital`), and server paths unchanged.**
...
## Notes
- **Product decision (2026-09-22): Picks and Performance are removed. DEPLOYED 2026-09-22 (commit e28445f on the box; live landing guard green).** Luo Capital is a screener — two calculator modes and a Tradebook — sold later as a flat membership for all features, with the paywall built last; nothing is gated today. App: the tab strip is Screener · Tradebook, no plan pill, `lc/LockedTeaser.jsx` deleted, `/picks` and `/performance` redirect to `/app`; the entitlement scaffold (`app_metadata.plan`, `getPlan`, `useAuth().plan`) stays in code and renders nothing. Landing: "What's inside" is a two-card section (Screener · Tradebook) with no tier tags; the guard in `landing.mjs` bans Picks / Performance / model book / Paid / SPY / "the book's return". The research (shadow logging via `select_with_shadow()`, the dual-gate admits, the model book, `docs/private/`) continues untouched as internal work. PRODUCT.md records the decision, the commercial model, and the copy that changes on paywall day.
- **Landing: "One structure. Two ways to set it." — DEPLOYED 2026-09-16 (commit 4c03ebc on the box; live guard green, header grid holds).** A new `#modes` section between "Three moves" and "What one setup looks like": Income and Upside cards in the step-card grammar (the app's own titles and lines, mini payoff curves, a 2×2 on the NVDA sample with identical $16,500 collateral, "Sample setup · illustrative"), one closing line, no new button. Shape-only copy by owner decision: no backtest / evidence / calculator language and no outcome-implying phrases anywhere on the page — `web/tests/e2e/landing.mjs` is the grep guard (plus stacking under 720, header shape, 1440/390 captures). The hero lede and the Screener preview copy broadened to name both aims; "and probability" left the preview (no such control exists). **Critique fix round (9bbd09c, owner's choices):** mode curves capped to the step-card scale (`max-width: 340px`) with "+$540 credit" / "+$2,040 max" annotations so the Income curve is not a copy of step 3; the Upside shelf sits above the breakeven line; the fourth KPI tile is a shape fact on both cards ("Stock must reach $200 · +8.7% from $184" / "$235 · +27.7%") instead of two different ratios; each card carries "NVDA at $184 · Oct 17" and one "Sample setups · illustrative" pill serves the section; the "Three moves" lede is back to shape-only; the compact NVDA card beside the ranked table was removed (the table stands alone with an "Income" pill) and the dashboard is retitled "What a row opens into" with an "Income" pill (the sample is Income-only and now says so); the gauge card's label / value / note stack (they had rendered inline since the relabel). Critique trend **21 → 24/32** (dual-agent; detector 0 anti-patterns; snapshots local). **Polish round (4c03ebc, owner's list):** the dashboard payoff labels are legible at 390 (a media query sets the 600-unit viewBox's text to 24 units, ≈10px on a phone, with the two annotations nudged clear of the geometry); the hero's back cards say "Shorts worthless" (no "P(profit)" survives on the page; "Chance of max profit ≈ 30%" in the gauge note is the app's own triad wording and stays); the table ranks "by credit as a share of max profit"; the mode cards' Max profit tiles read "per contract" and the reach tile owns the when; the mode curves are drawn with their own proportions (Income: bigger credit shelf, lower cap; Upside: small shelf clear of the breakeven dash, high cap) with every label clear of its marker; the mode cards bottom-align their curve and tiles; the step cards no longer lift on hover; "three-leg" carries a non-breaking hyphen. Brief: `.impeccable/surfaces/design-landing.md` (local); critique snapshots in `.impeccable/critique/` (local). Deploy is a pull + restart (Flask reads the landing HTML at startup).
- **Screener Upside mode — DEPLOYED 2026-09-16 (commit 894e655 on the box).** The same three legs built for a wide call spread; backend keyword params default-preserving (golden byte-identity test on the default path; cron/replay/CLI unchanged), `/api/run mode`, `scan_runs.mode` + `tradebook.mode` (migration applied), gauge relabelled to the exact 1 − δ_B − δ_C with ≈ δ_B and ≈ δ_C, no probability control in either mode (the 50% gate is a fixed strategy term; Upside sends it off), no cross-mode rows (the toggle is the side-by-side), no helper texts under the controls, calculator tag retired. Critique 24 → 27 (the second run pre-dated the withdrawals). Deferred: the 52-week ceiling (assessed; see memory), 1100 layout, ragged control labels.
- **Login / Create account rebuilt on the v1 design system — DEPLOYED 2026-09-13 (commit 897049d on the box).** One card, four states (Log in · Create account · Reset your password · Set a new password), forgot-password wired, return-to through the route guard, errors in the product's register, mobile (390) in scope; the landing header button reads "Log in / Sign up" → `/login`. Critique trend **30 → 35/40** (dual-agent; detector 0 findings; the re-run's five findings fixed after it). Verified live with all five `web/tests/e2e/` scripts. System-level changes made in this pass: `lc-loss-ink` for text on the loss tint, inputs and tabs keep a 12px radius on focus, autofill painted in the palette (DESIGN.md updated). Kept by owner decision: the landing's 1200px container (landing → login wordmark shift at wide viewports).
- **Tradebook + trade editor rebuilt on the v1 design system — DEPLOYED 2026-09-13 (commit a07c773 on the box).** Built from the approved shape (`.impeccable/surfaces/web-src-pages-tradebookpage-jsx.md`, replace-on-save) in stages (a)–(h), then two critique-fix rounds; see "Tradebook — rebuilt 2026-09-12" under Web UI. Critique trend **29 → 33/40** (dual-agent; detector 0 findings; snapshots in `.impeccable/critique/`). Verified live with all four `web/tests/e2e/` scripts. The payoff curve lost its axis captions on the owner's call (strike labels + zone legend carry the reading). Deferred: payoff-curve label crowding when the settled price sits above the short call; the 1100 column split (open panel ~2 viewports tall); the read-only editor still prints P(max)/spread for settled trades; the pending panel restates its status; "Back to default" drops focus; the third header click resets the sort silently.
- **Screener rebuilt on the v1 design system (2026-09-12) — DEPLOYED 2026-09-12/13.** `/app` was rebuilt from the approved shape (`.impeccable/surfaces/web-src-app-jsx.md`) in staged commits (a)–(g), a critique-fix round, then the dual-gate / grouping / chip-cause round and a controls-bar rebuild; see "Screener — rebuilt 2026-09-12" under Web UI. Impeccable critique trend: **17/40 → 29/40 → 35/40** (dual-agent re-runs; detector 0 findings; contrast 0 offenders; snapshots in `.impeccable/critique/`). The four baseline P1s are closed. Tests: `web/tests/e2e/` (`npm run test:e2e -- <baseUrl>`), run locally and against https://trigram.trade after each deploy. Follow-ups recorded: sub-1100 layout (design problem, not a squeeze); Tradebook adopts the shell next; per-ticker earnings flags and per-ticker scan progress need `/api/run` additions; a server-side unique constraint on `(user_id, scan_id, result_id)` for Save; rows as `role=grid` with `aria-activedescendant`; the "+N more" row outside the ↑/↓ path; the payoff label vs spot-line collision; the 50% P(max) default sits at the strategy's structural 50–55%.
- The platform runs a single algorithm: the Call Spread Risk Reversal screener (`screener.py`, served by `/api/run`)
- **Phase E shadow clock STARTED (2026-09-08 first scan).** The live sector scan logs a ROC (return-on-collateral) shadow ranking via `select_with_shadow()` — production picks/site/tradebook byte-identical (unit-enforced); shadow rows are plain runners-up; ROC-best derived at eval time. **Minimum 4–8 weeks of shadow data before any capital/ranking decision** — graded per-collateral-day + tail vs live outcomes (RANKER_SPEC §5c).
- **Dual-gate shadow extension (same 2026-09-08 clock):** the live scan also evaluates down to a $1.00 floor and logs sub-$5 admits passing the 1% ROC gate (top-5 by ROC per picked sector) as shadow rows — production byte-identical (`tests/test_shadow_gate.py`); admits derivable as `net_premium < min_net_premium`. The prespecified blend candidate was evaluated once on the v2 corpus and **REJECTED** (13.8 vs 20.3%/yr) — it does not shadow; no further blend variants on that dataset.
- **Universe expansion: tested once and REJECTED (2026-09-13).** The prespecified 240-name replay (`backtest3_*`, ~124k rows, `scripts/v3_evaluation.py`) **failed all three prestated conditions** (return 17.5 vs 20.7%/yr, concentration worsened 92→101% top-6, tail worse p5 −90 vs −52) — the dual gate self-selects the vol-richest additions (HOOD/COIN/CVNA, −$8.1M in aggregate) and the 240 random baseline is intrinsically below 118's. **The 118-name scan universe stands; no threshold-fishing on this dataset.** ROI figures drift slightly as labels complete — the findings doc's latest tables are authoritative (currently B 20.7 vs A 11.3%/yr on 118 at slot-#1).
...
- **Decided (2026-08-26) — NO-BUILD on pod-selector upgrades (scoreboard/bandit, parallel ranged prefetch).** A controlled probe (2026-08-25) showed the post-incident throughput ceiling is **account-level pacing** — concurrent streams don't sum on one pod, across pods, or across source networks — so client-side selection can't raise it. Evidence + full verdict: `docs/private/POD_SELECTION_CASE_STUDY.md` §7 and `docs/private/probe_results_2026-08-25.json`. A TTFB sanity-skip remains a documented dormant option. The lever for the ceiling is the vendor escalation.
