...
- **`web/src/pages/LoginPage.jsx`** — dark-themed **split-screen** sign-in page at `/login`; redirects to `/` if already logged in. **Left half:** vertically-centered form — Luo Capital lockup, "Welcome back." heading + "Sign in to the screener." subline, Email + Password fields (password has a Show/Hide toggle, UI-only), a full-width accent-purple **Sign in** button, and a muted `© 2026 · Luo Capital` footer. No social login, no sign-up link, no marketing copy. **Right half:** a darker brand panel with an SVG-only candlestick motif (slate wicks, profit/loss bodies, faint grid, faint purple radial glow) whose candles drift slowly via the `.candle-a/b/c` keyframes in `index.css` (~6–8s, honors `prefers-reduced-motion`); a centered `Luo Capital / Options Screener` lockup sits over it. No WebGL / video / external images. **Responsive:** the right panel is `hidden lg:block`, so narrow screens show the full-width centered form only. **Auth is unchanged** — the form still calls `supabase.auth.signInWithPassword` (the `handleSubmit` / `mode` / redirect logic was preserved verbatim; `signUp` branch retained though no UI exposes it), uses the existing `useAuth`, and routes into the screener on success; a failed login renders as a clean inline `text-loss` message. All styling uses the design tokens (`bg-base`, `bg-surface-raised`, `border-subtle`, `text-primary/secondary/tertiary`, `bg-accent`, `text-loss`).
...
The screener page (`App.jsx` route `/`) is locked to viewport height so the Header, the slim scanning-chips row, and the table's metadata bar always stay in view. Only the table body scrolls. The four scan controls live in a **collapsible left drawer** (closed by default — see `ControlsDrawer.jsx`), opened by the `⚙ Controls` trigger in the header; Run Scan stays in the header. The table legend is **collapsed by default** (toggled from the metadata bar). The macro-events band has been **removed from the UI** (the macro data is still in the scan response — see below).
...
- **`Header.jsx`** — route-aware header (uses `useLocation`); rendered independently by each page component:
  - `/` (screener): a **contained surface panel** (`bg-surface`, rounded, `border-subtle`) laid out as **three always-visible zones in a flex row**: **left** = branding lockup (`Luo Capital / Options Screener`, fixed); **center** = the always-on `TickerTape` (`flex-1`, fills the gap between the two fixed groups, hidden below `md`); **right** = the action cluster (fixed) — market badge → **`⚙ Controls`** (toggles the left controls drawer; `onToggleControls` prop, accent border when `controlsOpen`) → Clear → **Run Scan** (the `bg-accent` primary, amber when stale) → Tradebook → a divided, muted `Last run {timestamp}` + `Log out`. (There is **no hover-swap** — an earlier iteration cross-faded the tape and controls on hover; that was replaced by this always-visible three-zone layout.)
...
- **`ControlsDrawer.jsx`** — the four scan controls (Tickers, Weeks range, Min Net Premium, Min P(Profit)) in a panel that **slides in from the left**, over a click-to-close backdrop. **Closed by default** (`drawerOpen` defaults `false`); opened by the header's `⚙ Controls` trigger; auto-closes when a scan runs. Run Scan is **not** in here (it stays in the header). Pure presentation — every value/handler is owned by `App.jsx` and threaded through props (mirrors the old inline controls panel, laid out vertically and full-width). Enter in any field calls `onRun`.
...
- **`TickerTape.jsx`** — the always-visible scrolling market strip in the **center zone of the screener header**. Same embed mechanics as `TradingViewChart` (official `embed-widget-ticker-tape.js` script injected into a container div), but **mounted once** — a `useEffect` with empty deps, so it never reloads/reflickers (the header layout no longer hovers or unmounts it). **Config:** `colorTheme: "dark"`, `isTransparent: true` (drops the widget's own bg/border so it blends into the header surface), `showSymbolLogo: true`, `displayMode: "adaptive"`, `locale: "en"`. **Symbols:** `AMEX:SPY`, `NASDAQ:QQQ`, **`CAPITALCOM:VIX`** (the Capital.com CFD proxy — it renders on the free embed where `TVC:VIX` and `CBOE:VIX` did **not**), then the watchlist (`MU, NVDA, AMD, META, AVGO, PLTR, APP, TSM, GEV, TSLA`) built through the shared `toTvSymbol` map. In the header its wrapper is `pointer-events-none` (non-interactive strip — never steals clicks from the controls) and height-constrained (`h-9`, `overflow-hidden`, vertically centered) so it sits within the existing bar height; `hidden md:flex` so a narrow window drops the tape and keeps the lockup + controls.
