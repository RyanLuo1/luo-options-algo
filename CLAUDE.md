...
- **`/api/run`:** `mode: "income" | "upside"` (default income; anything else is a 400). The Income path calls `scan_ticker` with no new kwargs (byte-identical to before). The Upside preset: leg B Δ 0.05–0.20, `min_premium` default $0.00 (a debit never qualifies), `min_upside` default 0.05 (a request parameter, 0–5), the P gate unchanged. Every ranked row carries `mode`; the response adds `mode_used`, `min_upside_used`, `leg_b_delta_used`; `ticker_reasons` gains `below_min_upside` and the code `min_upside`.
- **Logging and the ledger:** `scan_runs.mode` and `tradebook.mode` (`docs/scan_mode_migration.sql`, owner-run). The server writes the column **only for upside rows**, so income inserts are unchanged whether or not the DDL has run; an upside save before the DDL fails loudly rather than landing as an income trade. `ml_dataset`, `sector_scan.py`, `replay_scan.py` and the backfills are untouched; web scans stay out of the corpus.
- **UI:** a two-segment mode control at the head of the controls bar with two-line help per mode ("Get paid to wait." / "Own the upside."). Switching swaps only the gate row: Income keeps Minimum credit ($100) + Minimum return on collateral (1%, live); Upside shows Minimum credit ($0) + **Minimum upside per $ of collateral** (5%, rescan to apply). Each mode keeps its own credit floor and its own last result (`useOptionsData(mode)` stores results per mode; sessionStorage holds `{ results }`). Upside: the "Not backtested — calculator only" pill on the results header and the panel, the metric bar = max profit ÷ collateral (a prop: `metric` / `metricLabel` on `RankedTable`), a **Move to max** column ((K_B − spot) / spot, descriptive), the Max-profit card highlighted instead of Credit. **Upside is a sorted list, not a ranking** (critique fix 2026-09-16): heading "Upside setups", status "Sorted by max profit per $ of collateral" (its default order, applied client-side in `App.jsx`; Income keeps the scanner's order), dot rank cells, never the lime rank-1 marker. In grouped view a ticker with results in the other mode shows that mode's best as a labelled second head row (`data-kind="other"`; pills "Income setup" / "Upside setup", and "Upside · not backtested" when an Upside row sits inside the Income table, where it also carries its own metric bar via `otherMetric`), selectable and saveable with the right `scan_id`; expanders reveal the active mode's variants only. Never "pick" — that word belongs to the paid Picks tab. The ⓘ tooltips (`InfoTip` in `lc/ui.jsx`) open leftward near the viewport's right edge; the P control's helper reads "Gates ~5 pts above the chance shown." The gauge is two labelled figures across both stat columns. A failed save renders a plain sentence (the server maps the missing-`mode`-column case to "The Tradebook can't store Upside trades until its mode column exists…"; the client never renders a message containing braces).
