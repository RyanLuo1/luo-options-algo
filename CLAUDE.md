...
- **`luo-capital-screener-state`** — App.jsx control state, written via a single `useEffect` whose deps include every persisted field. Persisted fields:
  ...
  - `selectedSetup` (explicit table-row override; `null` → panel shows the overall rank-1 default)
...
The screener page (`App.jsx` route `/`) is locked to viewport height so the Header, control bar (with the folded-in scanning chips), and the table's metadata bar always stay in view. Only the table body scrolls. The macro-events band and the table legend are **collapsed by default** (toggled from the controls / metadata bar respectively), so they only occupy space when explicitly expanded.
...
- **`App.jsx`** — screener page only (not a router/layout); owns all scan state, chart state, and control logic
  - **Row-driven per-setup detail model.** A single state atom drives the detail panel: `selectedSetup` (an explicit table-row override — `null` = use the rank-1 default). Everything else is **derived**:
    - `baseRanked` = `ranked` filtered by the Holdings `activeTickers`; `tableRows` = `baseRanked` re-ranked `1..N`.
  ...
  - **Behavior:** on a new scan a `useEffect` (primed `lastRankedRef`) resets `selectedSetup` to `null` → the detail panel + chart **default to the overall rank-1 setup**, with the rank-1 row highlighted. `selectRow(row)` (row click) sets `selectedSetup` → SetupDetail and the chart switch to that specific setup. Removing a Holdings pill that is the current override clears it (falls back to rank-1). There is **no card filter** anymore — the per-ticker overview card strip was removed, so selection is purely by table row.
  ...
  - The detail-zone chart is the **TradingView Advanced Chart embed** (`TradingViewChart.jsx`), driven by `toTvSymbol(chartTicker)` — App fetches no chart data from our backend. `selectedSetup` is persisted to sessionStorage.
...
- **`OverviewCard.jsx`** — ⚠️ **no longer rendered** (the per-ticker overview strip was removed). The component file is kept for now but nothing imports it. It was one ticker's single best setup as a rich "top-pick" card driven by a `by_ticker[i]` entry (`{ ticker, best, count }`); the description below is retained for reference should the strip ever return. Pure presentation, **all colors/fonts from design tokens** (`bg-surface`, `border-subtle`/`border-accent`, `text-profit`, `loss-dim`/`profit-dim`, `.num`) — no hardcoded hex.
...
- **Per-ticker overview section** — ⚠️ **removed.** A horizontal strip of `OverviewCard`s ("Best per ticker") used to sit between the controls and `<main>`. It was deleted: the page no longer renders the strip, the `byTicker`/`overviewCards` derivation and the `cardFilter` / scroll-helper state were dropped from `App.jsx`, and selection is now driven purely by ranked-table row clicks. (`OverviewCard.jsx` is left in the tree but unused.)
- **`Holdings.jsx`** — dismissible "Scanning:" ticker pills shown after a scan; removing a pill instantly filters that ticker from the table (and clears the row selection if it pointed at that ticker). Renders as an **inline chips row** (no band wrapper) and is **folded into the controls band** as a full-width sub-row — it no longer occupies its own standalone band.
